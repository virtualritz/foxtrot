...
fn main() {
    ...
    // Count entities
    let entity_count: usize = exchange.data.iter()
    ...
    for (idx, entity) in exchange.data.iter()
    ...
    for entity in exchange.data.iter().flat_map(|section| section.entities.iter()) {
        let entity_str = format!("{:?}", entity);
        let entity_type = entity_str.split('(').next().unwrap_or("Unknown").to_string();
